...
predictPromoterBinding = function(query, motif.list, input.assembly, p.val,...) {
  ...
  scannedPromoters = TFBSTools::searchSeq(x = motif, subject = dna, min.score = '80%', mc.cores = 8L)
  ...
  result = with(result, cbind(result, reshape::colsplit(attributes, split = "\\;", names = c('TF', 'TF_class', 'TF_sequence')))) %>%
    dplyr::mutate(seqname = as.numeric(seqname)) %>%
    dplyr::select(-source, -feature, -frame, -attributes) %>%
    dplyr::rename(motif_start = start, motif_end = end, motif_strand = strand, motif_score = score)
  ...
  result2 = result2 %>%
    ...
    dplyr::rename(promoter = helper, query_strand = strand, query_chromosome = seqnames)
